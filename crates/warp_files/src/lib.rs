...
mod editor_file_reader;
pub mod text_file_reader;
pub use editor_file_reader::{read_content_for_editor, validate_editor_content};
...
impl FileModel {
    ...
    pub fn open(
        ...
        let future = ctx.spawn(
            async move {
                let contents = read_content_for_editor(&file_path_buf).await;
                (file_id, contents)
            },
