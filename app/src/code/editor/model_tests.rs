...
#[test]
fn symbol_pairing_respects_word_boundaries_and_escapes() {
    App::test((), |mut app| async move {
        initialize_deps(&mut app);
        for (text, offset, open, close, expected) in [
            ("word", 3, '(', ')', "wo(rd"),
            ("call", 5, '(', ')', "call()"),
            ("can", 4, '\'', '\'', "can'"),
            ("\\", 2, '"', '"', "\\\""),
            ...
            editor.update(&mut app, |editor, ctx| {
                editor.cursor_at(CharOffset::from(offset), ctx);
                editor.autocomplete_symbol(open, close, ctx);
