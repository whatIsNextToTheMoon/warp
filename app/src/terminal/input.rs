...
use warpui::{
    ...
    SingletonEntity, TypedActionView, View, ViewContext, ViewHandle, WeakViewHandle, end_trace,
    start_trace,
};
...
impl CompletionSources {
    ...
    fn uses_native(self) -> bool {
        matches!(
            self,
            Self::NativeOnly | Self::WarpThenNative | Self::NativeThenWarp
        )
    }

    fn for_trigger(self, trigger: CompletionsTrigger, is_path_token: bool) -> Self {
        if self == Self::WarpThenNative && trigger == CompletionsTrigger::Keybinding {
            if is_path_token {
                Self::WarpOnly
            } else {
                Self::NativeThenWarp
...
impl Input {
    ...
    fn show_ephemeral_error_toast(&self, message: &str, ctx: &mut ViewContext<Self>) {
        ...
        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
            toast_stack.add_ephemeral_toast(
                DismissibleToast::error(crate::i18n::ui_text(message)),
                window_id,
                ctx,
            );
        });
    ...
    fn block_cloud_handoff_if_model_unsupported(&self, ctx: &mut ViewContext<Self>) -> bool {
        ...
        ToastStack::handle(ctx).update(ctx, |ts, ctx| {
            ts.add_ephemeral_toast(
                DismissibleToast::error(crate::i18n::ui_str(
    ...
    fn run_completions_async(
        ...
        let current_token = before_cursor_text
            .rsplit_once(char::is_whitespace)
            .map(|(_, token)| token)
            .unwrap_or(&before_cursor_text);
        let is_path_token =
            is_path_like_completion_token(current_token, self.path_separators(ctx).all);
        comp_sources = comp_sources.for_trigger(completions_trigger, is_path_token);
            ...
            match completions_trigger {
                CompletionsTrigger::Keybinding | CompletionsTrigger::SlashCommandAutoOpen
                    if !use_native_shell_completions
                        || (comp_sources == CompletionSources::NativeThenWarp
        ...
        let dispatch_native_up_front = matches!(
            comp_sources,
            CompletionSources::NativeOnly | CompletionSources::NativeThenWarp
        );
        ...
        let abort_handle = ctx
            .spawn_abortable(
                async move {
                    let native_results = if comp_sources == CompletionSources::NativeThenWarp {
                        native_results_fut
                    ...
                    let native_suggestions = native_results
                    ...
                    let suggestions = if native_suggestions.is_some()
        ...
        ctx.spawn(
            ...
            move |input, result, ctx| {
                if let Err(error) = result {
                    ...
                    let error = crate::i18n::ui_text(&error);
                    let message = if crate::i18n::is_chinese_locale() {
                        format!("无法上传附件：{error}")
                    } else {
                        format!("Couldn't upload attachment: {error}")
                    };
                    let window_id = ctx.window_id();
                    ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                        toast_stack.add_ephemeral_toast(
