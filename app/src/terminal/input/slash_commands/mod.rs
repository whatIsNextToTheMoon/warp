            ...
            SlashCommandKind::CopyDebuggingId => {
                ...
                match debugging_payload {
                    Some(debugging_payload) => {
                        ...
                        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                            toast_stack.add_ephemeral_toast(
                                DismissibleToast::default(crate::i18n::ui_str(
                                    "Debugging information copied to clipboard",
                                )),
                                window_id,
                                ctx,
                            );
                    ...
                    None => show_error_toast(
                        crate::i18n::ui_str("No debugging ID available for this conversation yet."),
                        ctx,
                    ),
                }
            }
