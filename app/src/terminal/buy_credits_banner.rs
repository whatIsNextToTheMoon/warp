    ...
    fn update_addon_credits_options(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let items = self
            ...
            .map(|(index, option)| {
                ...
                let primary_text = if crate::i18n::is_chinese_locale() {
                    format!("{price_label} / {} 点数", option.credits)
                } else {
                    format!("{price_label} / {} credits", option.credits)
                };
    ...
    fn render_checkout_pending(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let banner_text = Flex::column()
            .with_children([
                appearance
                    .ui_builder()
                    .paragraph(crate::i18n::ui_str("Finish your purchase in the browser"))
                    .with_style(UiComponentStyles {
                        font_size: Some(14.),
                        ..Default::default()
                    })
                ...
                appearance
                    .ui_builder()
                    .paragraph(crate::i18n::ui_str(
                        "Credits will be added to your account after checkout.",
                    ))
                    .with_style(UiComponentStyles {
                        font_color: Some(theme.sub_text_color(theme.surface_1()).into()),
                        ..Default::default()
