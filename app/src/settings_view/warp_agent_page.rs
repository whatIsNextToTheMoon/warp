            ...
            Err(err) => {
                ...
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast = DismissibleToast::error(
                        crate::i18n::ui_str("Couldn't start Grok login: {err}")
                    ...
                    ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                        toast_stack.add_ephemeral_toast(
                            DismissibleToast::error(
                                crate::i18n::ui_str("Couldn't connect SuperGrok: {err}")
                                    .replace("{err}", &err.to_string()),
                            )
                            .with_object_id(GROK_OAUTH_CONNECT_TOAST_OBJECT_ID.to_string()),
                            window_id,
...
fn render_global_ai_toggle(
    ...
    if is_anonymous && !can_use_local_byok {
        row.add_child(
            Flex::row()
        ...
        row.add_child(
            Container::new(
                ui_builder
                    ...
                    .check(if is_anonymous && can_use_local_byok {
    ...
    fn render_grok_subscription_row(
        ...
        if let Some(tokens) = grok_tokens {
            let connected_text = match tokens.connected_at.map(DateTime::<Local>::from) {
                Some(connected_at) => crate::i18n::ui_str("Connected on {}.")
                    .replace("{}", &connected_at.format("%Y-%m-%d %H:%M").to_string()),
                // Tokens stored before the connection time was tracked.
                None => crate::i18n::ui_str("Connected."),
            };
...
impl CustomInferenceVisibility {
    ...
    fn compute(team_scope: &TeamContext<'_>, app: &AppContext) -> Self {
        ...
        let is_any_ai_enabled = AISettings::as_ref(app).is_any_ai_enabled(app);
        let is_byo_enabled =
            workspaces.is_byo_api_key_enabled(app) || FeatureFlag::SoloUserByok.is_enabled();
