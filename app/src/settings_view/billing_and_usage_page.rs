...
pub(crate) fn render_premium_upgrade_savings_note(
    ...
    let fragments = if crate::i18n::is_chinese_locale() {
        vec![
            FormattedTextFragment::hyperlink("升级到 Build 计划", upgrade_url),
            FormattedTextFragment::plain_text(format!("可为附加点数节省 {percent}。")),
        ]
    } else {
        vec![
        ...
        match event {
            ...
            UserWorkspacesEvent::PurchaseAddonCreditsCheckoutRequired { checkout_url } => {
                if self.purchase_addon_credits_loading {
                    ...
                    self.show_toast(
                        &crate::i18n::ui_str(CHECKOUT_PENDING_MESSAGE),
                        ToastFlavor::Default,
                        ctx,
                    );
