...
impl SettingsWidget for CLIAgentWidget {
    ...
    fn search_terms(&self) -> &str {
        "third party cli coding agent claude codex gemini toolbar footer quick actions show layout chip chips rearrange re-arrange bar command regex auto rich input dismiss plain terminal"
    }
...
impl ApiKeysWidget {
        ...
        let provider_api_key_editors = LLMProvider::API_KEY_PROVIDERS
            ...
            .map(|provider| {
                ...
                AISettingsPageView::update_editor_interaction_state(
                    editor.clone(),
                    is_any_ai_enabled && local_byo_enabled && member_byo_keys_allowed,
                    ctx,
                );
                ...
                ctx.subscribe_to_model(&workspace_handle, move |_, workspace, event, ctx| {
                    if let UserWorkspacesEvent::TeamsChanged = event {
                        ...
                        let local_byo_enabled =
                            is_byo_enabled || FeatureFlag::SoloUserByok.is_enabled();
                        let is_enabled = is_any_ai_enabled && local_byo_enabled;
    ...
    fn render_api_key_input(
        ...
        let label = crate::i18n::ui_text(&label);
...
impl CustomInferenceVisibility {
    fn compute(app: &AppContext) -> Self {
        ...
        let show_provider_keys = member_byo_keys_allowed || FeatureFlag::SoloUserByok.is_enabled();
        let provider_keys_enabled = show_provider_keys && is_any_ai_enabled && local_byo_enabled;
...
impl SettingsWidget for ApiKeysWidget {
    ...
    fn render(
        ...
        if (is_byo_enabled || FeatureFlag::SoloUserByok.is_enabled()) && show_provider_keys {
            column.add_child(
