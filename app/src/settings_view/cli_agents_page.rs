...
use crate::settings::{
    ...
    SubmitRichInputOnCtrlEnter, TreatManualCodexAsPlainTerminal,
};
...
impl CLIAgentsPageView {
    ...
    fn build_page() -> PageType<Self> {
        let widgets: Vec<Box<dyn SettingsWidget<View = Self>>> = vec![
            ...
            Box::new(ManualCodexPlainTerminalWidget::default()),
            ...
            .map(|(pattern_clone, current_agent)| {
                ctx.add_typed_action_view(move |ctx| {
                    ...
                    dropdown.set_menu_header_text_override(|label| {
                        if label == "Other" {
                            crate::i18n::ui_str("Select coding agent")
                        } else {
                            label.to_string()
                        }
                    });
...
impl TypedActionView for CLIAgentsPageView {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            CLIAgentsPageAction::ToggleTreatManualCodexAsPlainTerminal => {
                AISettings::handle(ctx).update(ctx, |settings, ctx| {
                    report_if_error!(
                        settings
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::new(
                "plain terminal mode for manually launched Codex",
                builder(SettingsAction::CLIAgents(
                    CLIAgentsPageAction::ToggleTreatManualCodexAsPlainTerminal,
                )),
                context,
                flags::MANUAL_CODEX_PLAIN_TERMINAL_FLAG,
            )
...
#[derive(Default)]
struct ManualCodexPlainTerminalWidget {
    toggle: SwitchStateHandle,
}

impl SettingsWidget for ManualCodexPlainTerminalWidget {
    type View = CLIAgentsPageView;

    fn search_terms(&self) -> &str {
        "codex manual plain terminal rich input"
    }

    fn render(&self, view: &Self::View, _: &Appearance, app: &AppContext) -> Box<dyn Element> {
