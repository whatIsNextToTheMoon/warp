...
pub fn status_message_naming_model(message: &str, model_display_name: &str) -> String {
    if crate::i18n::is_chinese_locale() {
        return format!("{}（{model_display_name}）", crate::i18n::ui_text(message));
    }
            ...
            .is_none_or(|output| output.get().messages.is_empty())
    {
            ...
            .map(|action| &action.action)
        {
            ...
            Some(AIAgentActionType::CallMCPTool {
                ...
                {
                    Some(server) if crate::i18n::is_chinese_locale() => {
                        format!("正在调用 {server} 上的 MCP 工具“{name}”...")
                    }
                    Some(server) => format!("Calling \"{name}\" MCP tool on {server}..."),
                    None if crate::i18n::is_chinese_locale() => {
                        format!("正在调用 MCP 工具“{name}”...")
                    }
