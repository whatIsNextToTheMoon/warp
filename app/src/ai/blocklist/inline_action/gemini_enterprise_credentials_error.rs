...
impl GeminiEnterpriseCredentialsErrorView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        let refresh_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(crate::i18n::ui_str("Refresh credentials"), PrimaryTheme)
        ...
        let manage_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(crate::i18n::ui_str("Manage"), NakedTheme)
                .with_size(ButtonSize::InlineActionHeader)
                .on_click(|ctx| {
    ...
    fn update_refresh_button(
        ...
        self.refresh_button.update(ctx, |button, ctx| {
            button.set_label(crate::i18n::ui_str(label), ctx);
...
impl View for GeminiEnterpriseCredentialsErrorView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let title = if self.refresh_succeeded {
            crate::i18n::ui_str("Gemini Enterprise credentials refreshed")
        } else if self.refresh_requested {
            crate::i18n::ui_str("Refreshing Gemini Enterprise credentials...")
        } else {
            crate::i18n::ui_str("Gemini Enterprise credentials expired or invalid")
        };
        let detail = if self.refresh_succeeded {
            crate::i18n::ui_str("Your credentials are ready. Retry the request to continue.")
        } else if self.refresh_requested {
            crate::i18n::ui_str("Warp is refreshing your Google Cloud credentials.")
        } else {
            crate::i18n::ui_str(
        ...
        let make_header = || {
            Flex::row()
                ...
                .with_child(
                    Text::new(title.clone(), appearance.ui_font_family(), 14.)
        ...
        let make_detail = || {
            Text::new(detail.clone(), appearance.ui_font_family(), 14.)
