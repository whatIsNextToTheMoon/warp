...
fn mcp_blocked_title_text(tool_name: &str, server_name: Option<&str>) -> String {
    if tool_name.is_empty() {
        return crate::i18n::ui_str(MCP_TOOL_WAITING_FOR_USER_MESSAGE);
    }
    if crate::i18n::is_chinese_locale() {
        return match server_name {
            Some(server) => format!("可以调用服务器 {server} 上的 MCP 工具 {tool_name} 吗？"),
            None => format!("可以调用 MCP 工具 {tool_name} 吗？"),
        };
    }
    match server_name {
...
fn mcp_viewing_detail_title_text(tool_name: &str, server_name: Option<&str>) -> String {
    if tool_name.is_empty() {
        return crate::i18n::ui_str(VIEWING_MCP_TOOL_DETAIL_MESSAGE);
    }
    if crate::i18n::is_chinese_locale() {
        return match server_name {
            Some(server) => format!("正在查看 {server} 上的 MCP 工具 {tool_name}"),
            None => format!("正在查看 MCP 工具 {tool_name}"),
        };
    }
    match server_name {
